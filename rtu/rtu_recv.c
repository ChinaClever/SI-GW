 ...
 * 返回值：12 正确
 */
static uchar *rtu_recv_data(uchar *ptr, int line, sDataPacket *packet)
{
 ...
 * 返回值：true
 */
static boolean rtu_recv_crc(uchar *buf, int len, sRtuRecv *msg)
{
	boolean ret = true;
...
boolean rtu_recv_packet(uchar *buf, int len, sRtuRecv *pkt)
{
	...
	if(res > 0) {
		...
		pkt->data->id = pkt->addr;

		ptr = rtu_recv_data(ptr, res, pkt->data);
